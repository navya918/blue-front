...
const EmployeeHomePage = ({
    ...
    const fetchSubmissions = async () => {
      try {
        const response = await axios.get(
          `https://manibackendts-f7fee3fvgxdchebf.canadacentral-01.azurewebsites.net/api/timesheets/list/${employeeId}`,
        );
  ...
  const handleDeleteTimesheet = async (id) => {
    try {
      await axios.delete(
        `https://manibackendts-f7fee3fvgxdchebf.canadacentral-01.azurewebsites.net/api/timesheets/delete/${id}`,
      );
  ...
  const filterSubmissions = (status) =>
    setFilteredSubmissions(
      status ? submissions.filter((sub) => sub.status === status) : submissions,
    );
      ...
      <Nav className="flex-col mr-4">
        ...
        >
          <h5 className="text-gray-500">
            Pending Timesheets: {counts.pending}
          </h5>
        </Nav.Link>
        <Nav.Link
        ...
        >
          <h5 className="text-green-500">
            Approved Timesheets: {counts.approved}
          </h5>
        </Nav.Link>
        <Nav.Link
        ...
        >
          <h5 className="text-red-500">
            Rejected Timesheets: {counts.rejected}
          </h5>
        </Nav.Link>
      </Nav>
                    ...
                    <td>
                      {submission.status !== "APPROVED" &&
                        submission.status !== "REJECTED" && (
                          <>
                            <Button
                              className="bg-blue-500 text-white font-10 text-sm m-1 p-1"
                              onClick={() => handleEditTimesheet(submission)}
                            >
                              Edit
                            </Button>
                            <Button
                              className="bg-red-500 text-white font-bold text-xs rounded-lg p-1"
                              onClick={() =>
